classdef (Abstract) hNRScheduler < handle
    ...
    %   start symbol is DL so that output can be immediately conveyed to
    %   UEs in DL direction, assuming zero run time for scheduler
    %   algorithm. Hence, in FDD mode the schedulers (DL and UL) run
    %   periodically (configurable) as every slot is DL while for TDD DL
    %   time is checked. In FDD mode, schedulers run to assign the
    ...
    properties
        ...
        %CSIMeasurements Reported CSI measurements
    ...
    methods
        function obj = hNRScheduler(param)
            ...
            % PDSCHDMRSConfigurationType   - PDSCH DM-RS configuration type (1 or 2)
            % PDSCHDMRSLength              - PUSCH DM-RS length (1 or 2)
            % PDSCHDMRSAdditionalPosTypeA  - Additional PDSCH DM-RS positions for Type A (0..3)
            % PDSCHDMRSAdditionalPosTypeB  - Additional PDSCH DM-RS positions for Type B (0 or 1)
            ...
            for i=1:numUEs
                ...
                obj.GNBTxAntPanel = param.GNBTxAntPanelSize;
                obj.GNBRxAntPanel = param.GNBRxAntPanelSize;
                obj.UEsRxAntPanel(i, :) = param.UERxAntPanelSize;
                obj.UEsTxAntPanel(i, :) = param.UETxAntPanelSize;
            ...
            for i=1:numUEs
                ...
                if isfield(param, 'SubbandSize') && ~isempty(param.SubbandSize)
                    numSubbands = ceil(obj.NumPDSCHRBs/param.SubbandSize);
                    % Initialize i11, i12, i13, i2 for each UE
                    obj.CSIMeasurement(i).PMISet = struct('i1', [1 1 1], 'i2', ones(numSubbands,1));
                else % Wideband PMI
                    obj.CSIMeasurement(i).PMISet = struct('i1', [1 1 1], 'i2', 1);
                end
                % Initialize DL channel quality as CQI index 7
                obj.CSIMeasurement(i).CQI = 7*ones(1, obj.NumPDSCHRBs);
            end
            
            % Initialize UL channel quality as CQI index 7
            ...
            if isfield(param, 'CSIRSRowNumber')
                ...
                csirsConfig.RowNumber = 2; % SISO case
            end
            % Validate the number of CSI-RS ports for the given gNB antenna
            % configuration
            if csirsConfig.NumCSIRSPorts > param.GNBTxAnts
                error('nr5g:hNRScheduler:InvalidCSIRSRowNumber',...
                    'Number of CSI-RS ports (%d) corresponding to CSI-RS row number (%d) must be less than or equal to GNBTxAnts (%d).',...
                     csirsConfig.NumCSIRSPorts, csirsConfig.RowNumber, param.GNBTxAnts);
            end
            if isfield(param, 'PanelDimensions') 
                ...
                csiReportConfig.PanelDimensions = param.PanelDimensions;
            end
            % Set oversampling factors
            O1 = 1;
            O2 = 1;
            if(csirsConfig.NumCSIRSPorts > 2)
                ...
                configIdx = find(panelConfigs(1,:) == csiReportConfig.PanelDimensions(1) & panelConfigs(2,:) == csiReportConfig.PanelDimensions(2),1);
                if isempty(configIdx)
                    error('nr5g:hDLPMISelect:InvalidPanelConfiguration',['The given panel configuration ['...
                        num2str(reportConfig.PanelDimensions(1)) ' ' num2str(reportConfig.PanelDimensions(2)) '] is not valid. '...
            ...
            csiReportConfig.OverSamplingFactors = [O1 O2];
            % Set codebook mode
            if isfield(param, 'CodebookMode')
                validateattributes(param.CodebookMode, 'numeric', {'scalar', 'integer', '>', 0, '<', 3}, 'param.CodebookMode', 'CodebookMode')
                csiReportConfig.CodebookMode =  param.CodebookMode;
            else
                csiReportConfig.CodebookMode = 1;
            end
            % Set Codebook subset restriction
            if isfield(param, 'CodebookSubsetRestriction')
                csiReportConfig.CodebookSubsetRestriction =  param.CodebookSubsetRestriction;
            else
                csiReportConfig.CodebookSubsetRestriction = [];
            end
            % Set i2 restriction
            if isfield(param, 'i2Restriction')
                csiReportConfig.i2Restriction =  param.i2Restriction;
            else
                csiReportConfig.i2Restriction = [];
            end
            % Get Type-1 single panel codebook
            if csirsConfig.NumCSIRSPorts > 1
                % Codebooks for all possible ranks (1-8), limited by the CSI-RS port count
                maxRank = min(csirsConfig.NumCSIRSPorts, 8);
                obj.Type1SinglePanelCodebook = cell(maxRank, 1);
                for rank = 1:maxRank % For all ranks limited by the CSI-RS port count
                    obj.Type1SinglePanelCodebook{rank}= hPMIType1SinglePanelCodebook(csiReportConfig, rank); % Get precoding codebook
                end
            end
        end
        ...
        function cqiRBs = getChannelQualityStatus(obj, linkDir, rnti)
            ...
            if linkDir % Uplink
                ...
                cqiRBs = obj.CSIMeasurement(rnti).CQI;
            end
        end
        ...
        function updateChannelQualityStatus(obj, cqiRBs, linkDir, rnti)
            ...
            if linkDir % Uplink
                ...
                obj.CSIMeasurement(rnti).CQI = cqiRBs;
            end
        end
    ...
    methods (Access = protected)
        ...
        function [reTxUEs, updatedRBGStatus, ULGrants] = scheduleRetransmissionsUL(obj, scheduledSlot, startSym, numSym, rbgOccupancyBitmap)
            ...
            rank = ones(length(reTxAssignmentOrder), 1); % Rank 1
            ...
            for i = 1:length(reTxAssignmentOrder)
                ...
                if ~isempty(failedRxHarqs) % At least one UL HARQ process for UE requires retransmission
                    ...
                    [isAssigned, allottedRBGBitmap, MCS] = getRetransmissionResources(obj, obj.ULType, reTxAssignmentOrder(i), ...
                        lastTbsBits, updatedRBGStatus, scheduledSlot, startSym, numSym, rank(i));
                    if isAssigned
            ...
            for i = 1:length(reTxAssignmentOrder) % For each UE
                ...
                if ~isempty(failedRxHarqs) % At least one DL HARQ process for UE requires retransmission
                    ...
                    [isAssigned, allottedRBGBitmap, MCS] = getRetransmissionResources(obj, obj.DLType, reTxAssignmentOrder(i),  ...
                        lastTbsBits, updatedRBGStatus, scheduledSlot, startSym, numSym, rank);
                    if isAssigned
        ...
        function [newTxUEs, updatedRBGStatus, ULGrants] = scheduleNewTxUL(obj, scheduledSlot, eligibleUEs, startSym, numSym, rbgOccupancyBitmap)
            ...
            newTxGrantCount = 0;
            rank = ones(length(eligibleUEs), 1); % Rank 1
        ...
        function k1 = getPDSCHFeedbackSlotOffset(obj, PDSCHSlotOffset)
            ...
            % PDSCH feedback is currently suppoorted to be sent with
        ...
        function schedulerInput = createSchedulerInput(obj, linkDir, slotNum, eligibleUEs, selectedRank, rbgIndex, startSym, numSym)
            ...
            if linkDir % Uplink
                ...
                for i = 1:length(eligibleUEs)
                    channelQuality(eligibleUEs(i), :) = obj.CSIMeasurement(eligibleUEs(i)).CQI;
                end
        ...
        function [isAssigned, allottedBitmap, MCS] = getRetransmissionResources(obj, linkDir, rnti, ...
                tbs, rbgOccupancyBitmap, ~, startSym, numSym, rank)
            ...
            [~, nREPerPRB] = tbsCapability(obj, linkDir, rank, mappingType, startSym, ...
                numSym, 1, 'QPSK', mcsTable(1,2)/1024);
            for i = 1:numRBGs
                if ~rbgOccupancyBitmap(sortedIndices(i)) % Free RBG
                    ...
                    servedBits = nREPerPRB*numRBsRBG*mcsInfo(3); % Approximate TBS bits served by current RBG
            ...
            if isAssigned
                ...
                actualServedBits = tbsCapability(obj, linkDir, rank, mappingType, startSym, ...
                        numSym, grantRBs, modScheme, codeRate);
        ...
        function [rank, W] = selectRankAndPrecodingMatrix(obj, csiReport)
            %selectRankAndPrecodingMatrix Select rank and precoding matrix based on the CSI report from the UE
            %   [RANK, W] = selectRankAndPrecodingMatrix(OBJ, CSIREPORT)
            ...
            if isempty(obj.Type1SinglePanelCodebook)
                ...
                nCSIPorts = size(obj.Type1SinglePanelCodebook{rank}, 1);
                codebook = obj.Type1SinglePanelCodebook{rank};
                ...
                W = complex(zeros(rank, nCSIPorts, numPRGs));
        ...
        function modScheme = modSchemeStr(~, modSchemeBits)
            %modScheme Return the modulation scheme string based on modulation scheme bits
        ...
        function updateHARQContextDL(obj, grants)
            ...
            for grantIndex = 1:length(grants) % Update HARQ context
                ...
                harqProcess = hUpdateHARQProcess(obj.HarqProcessesDL(grant.RNTI, grant.HARQID+1), 1);
                obj.HarqProcessesDL(grant.RNTI, grant.HARQID+1) = harqProcess;
                obj.HarqStatusAndNDIDL(grant.RNTI, grant.HARQID+1, 1) = 1; % Mark HARQ process as busy
        ...
        function updateHARQContextUL(obj, grants)
            ...
            for grantIndex = 1:length(grants) % Update HARQ context
                grant = grants{grantIndex};
                harqProcess = hUpdateHARQProcess(obj.HarqProcessesUL(grant.RNTI, grant.HARQID+1), 1);
                obj.HarqProcessesUL(grant.RNTI, grant.HARQID+1) = harqProcess;
                obj.HarqStatusAndNDIUL(grant.RNTI, grant.HARQID+1, 1) = 1; % Mark HARQ process as busy
                obj.HarqStatusAndNDIUL(grant.RNTI, grant.HARQID+1, 2) = grant.NDI;
                if strcmp(grant.Type, 'reTx')
                    % Clear the retransmission context for this HARQ
                    % process of the selected UE to make it ineligible
                    % for retransmission assignments
