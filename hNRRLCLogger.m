classdef hNRRLCLogger < handle
    %hNRRLCLogger RLC statistics logging object
    ...
    methods (Access = public)
        function obj = hNRRLCLogger(simParameters, lchInfo)
            %hNRRLCLogger Construct an RLC logging object
            %
        ...
        function logCellRLCStats(obj, gNB, UEs)
            ...
            for ueIdx = 1:obj.NumUEs
                % Get RLC statistics
                obj.UERLCStats{ueIdx} = getRLCStatistics(UEs{ueIdx}, ueIdx);
                obj.GNBRLCStats{ueIdx} = getRLCStatistics(gNB, ueIdx);
            end
