classdef helperApplication < handle
%helperApplication Implement application layer functionality
%   APP = helperApplication creates an object for application layer. It is
%   a container of different application traffic generators and
%   application receiver to terminate the packets. It is responsible for
%   invoking different traffic model objects for generating packets. It
%   also maintains the application level statistics.
%
%   APP = helperApplication(Name, Value) creates an object for application
...
properties(Constant)
    ...
    ApplicationPacket = struct('MSDULength', 0, ...
        ...
        'DestinationMACAddress', '000000000000', ... % Immediate destination node MAC address
        'IsMeshFrame', false, ... % If true, frame DestinationID and DestinationMACAddress change for each hop
        'MeshSourceAddress', '000000000000', ... % Original packet source
        'MeshDestinationAddress', '000000000000', ... % Final destination
        'Timestamp', 0, ... % Packet generation time stamp at origin
        'Data', zeros(2304, 1, 'uint8'));
end

methods
    function obj = helperApplication(varargin)
        % Name-value pairs
        for idx = 1:2:nargin
    ...
    function addApplication(obj, app, metaData)
        %appApplication Add application traffic model to application layer
        %
        % appApplication(OBJ, APP, METADATA) adds the application traffic
        ...
        if obj.ApplicationsCount == obj.MaxApplications
            error('helperApplication:MaxAppLimit', ...
