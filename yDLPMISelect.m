function [PMISet,info] = yDLPMISelect(carrier,csirs,reportConfig,nLayers,H,varargin)
    ...
    if isempty(csirsIndSubs_k) || ~any(W(:))
        ...
        for SubbandIdx = 1:subbandInfo.NumSubbands
            ...
            if all(isnan(sinrValuesPerSubband(:))) % CSI-RS is absent in the subband
                ...
                % the PMI indices
                SubbandSINRs(SubbandIdx,:,:,:,:,:) = squeeze(entropicMean(entropicMean(sinrValuesPerSubband,'omitnan'),'omitnan'));
