classdef hNRGNBMAC < hNRMAC
    ...
    methods
        function obj = hNRGNBMAC(simParameters)
            ...
            % CSIRSSubcarrierLocation  - CSI-RS subcarrier number in the RB
            % CSIRSSymboLocation       - CSI-RS symbol location in the slot
            ...
            % Set non-zero-power (NZP) CSI-RS configuration. All the UEs
            ...
            elseif ~ismember(simParameters.GNBTxAnts, [1,2,4,8,16,32,64,128,256,512,1024])
                error('nr5g:hNRGNBMAC:InvalidAntennaSize',...
            ...
            % configuration
            if csirs.NumCSIRSPorts > simParameters.GNBTxAnts
                error('nr5g:hNRGNBMAC:InvalidCSIRSRowNumber',...
                    'Number of CSI-RS ports (%d) corresponding to CSI-RS row number (%d) must be less than or equal to GNBTxAnts (%d)',...
                     csirs.NumCSIRSPorts, csirs.RowNumber, simParameters.GNBTxAnts);
            end
            % Validate the CSI-RS configuration
            if isfield(simParameters, 'CSIRSSubcarrierLocation')
                csirs.SubcarrierLocations = simParameters.CSIRSSubcarrierLocation;
            else
            ...
            if isfield(simParameters, 'CSIRSSymbolLocation')
                csirs.SymbolLocations = simParameters.CSIRSSymbolLocation;
            else
        ...
        function controlRx(obj, pktInfo)
            ...
            switch(pktType)
                ...
                case obj.CSIReport % CSI report received containing RI, PMI and CQI
                    ...
                    obj.Scheduler.CSIMeasurement(pktInfo.RNTI) = csiReport;
                    % Assuming UL channel quality to be same as DL channel quality reported in CSI measurement report
                    obj.Scheduler.ChannelQualityUL(pktInfo.RNTI, :) = obj.Scheduler.CSIMeasurement(pktInfo.RNTI).CQI;
            end
        end
        ...
        function dlControlRequest(obj)
            ...
            if(obj.Scheduler.DuplexMode == 0 || ~isempty(find(obj.Scheduler.DLULSlotFormat(obj.Scheduler.CurrDLULSlotIndex + 1, :) == obj.DLType, 1)))
                dlControlType = [];
                dlControlPDUs = {};
                
                csirsLocations = obj.CsirsConfig.SymbolLocations; % CSI-RS symbol locations
                % If the mode is TDD then all the CSI-RS symbols must be DL symbols
                if ~(obj.Scheduler.DuplexMode == 1 && all(obj.Scheduler.DLULSlotFormat(obj.Scheduler.CurrDLULSlotIndex + 1, csirsLocations+1) ~= obj.DLType))
                    % Set carrier configuration object
                    carrier = nrCarrierConfig;
                    carrier.SubcarrierSpacing = obj.Scheduler.SCS;
                    carrier.NSizeGrid = obj.Scheduler.NumPDSCHRBs;
                    carrier.NSlot = obj.Scheduler.CurrSlot;
                    carrier.NFrame = obj.Scheduler.SFN;
                    csirsInd = nrCSIRSIndices(carrier, obj.CsirsConfig);
                    
                    if ~isempty(csirsInd) % Empty value means CSI-RS is not scheduled in the current slot
                        dlControlType(1) = hNRPhyInterface.CSIRSPDUType;
                        dlControlPDUs{1} = obj.CsirsConfig;
                    end
                end
    ...
    methods (Access = private)
        function [pduLen, type] = sendMACPDU(obj, rnti, downlinkGrant)
            ...
            pdschInfo.PDSCHConfig.DMRS = nrPDSCHDMRSConfig('DMRSConfigurationType', obj.Scheduler.PDSCHDMRSConfigurationType, ...
                'DMRSTypeAPosition', obj.Scheduler.DMRSTypeAPosition, 'DMRSLength', downlinkGrant.DMRSLength, ...
                'DMRSAdditionalPosition', dmrsAdditonalPos, 'NumCDMGroupsWithoutData', downlinkGrant.NumCDMGroupsWithoutData);
            ...
            carrierConfig.NFrame = obj.Scheduler.SFN;
            ...
            csirsLocations = obj.CsirsConfig.SymbolLocations; % CSI-RS symbol locations
            if obj.Scheduler.DuplexMode == 0 || all(obj.Scheduler.DLULSlotFormat(obj.Scheduler.CurrDLULSlotIndex + 1, csirsLocations+1) == obj.DLType)
                % (Mode is FDD) OR (Mode is TDD And CSI-RS symbols are DL symbols)
                pdschInfo.PDSCHConfig.ReservedRE = nrCSIRSIndices(carrierConfig, obj.CsirsConfig, 'IndexBase', '0based'); % Reserve CSI-RS REs
            end
        ...
        function rxRequestToPhy(obj, rnti, uplinkGrant)
            ...
            puschInfo.PUSCHConfig.DMRS = nrPUSCHDMRSConfig('DMRSConfigurationType', obj.Scheduler.PUSCHDMRSConfigurationType, ...
                'DMRSTypeAPosition', obj.Scheduler.DMRSTypeAPosition, 'DMRSLength', uplinkGrant.DMRSLength, ...
