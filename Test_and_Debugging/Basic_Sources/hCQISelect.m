...
% hCQISelect PDSCH Channel quality indicator (CQI) calculation
%   [CQI,PMISET,CQIINFO,PMIINFO] = hCQISelect(CARRIER,CSIRS,REPORTCONFIG,NLAYERS,H)
%   returns CQI and precoding matrix indicator (PMI) values, as defined in
...
%   NLAYERS, and estimated channel information H. The function also returns
%   the additional information about the signal to interference and noise
%   ratio (SINR) values that are used for the CQI computation and PMI
%   computation.
%
...
%                     subband sizes defined in TS 38.214 Table 5.2.1.4-2.
%                     It is applicable only when the either CQIMode or
%                     PMIMode are provided as 'Subband' and the size of BWP
%                     is greater than or equal to 24 PRBs
%   PRGSize         - Optional. Precoding resource block group (PRG)
%                     size for CQI calculation, provided by the
%                     higher-layer parameter pdsch-BundleSizeForCSI. This
%                     field is applicable for the CSI report quantity
%                     cri-RI-i1-CQI as defined in TS 38.214 Section
%                     5.2.1.4.2. It must be a scalar and it must be one of
%                     {2, 4}. Empty ([]) is also supported to represent
%                     that this field is not configured by higher layers.
%                     If it is present and not configured as empty, the CQI
%                     values are computed according to the configured
%                     CQIMode and the PMI value is reported for each PRG
%                     irrespective of PMIMode. The default value is []
%   CodebookMode    - Optional. It represents the codebook mode
%                     and it must be a scalar. The value must be one of {1,
%                     2}. It is applicable only when the number of
%                     transmission layers is 1 or 2 and number of CSI-RS
%                     ports is greater than 2. The default value is 1
...
%   CQI reporting mode is 'Subband', where numSubbands in the number of
%   subbands and numCodewords is the number of codewords. The first row
%   consists of 'Wideband' CQI value and if the CQI mode is 'Subband', the
%   'Wideband' CQI value is followed by the subband differential CQI values
%   for each subband. The subband differential values are scalars ranging
%   from 0 to 3 and these values are computed based on the offset level, as
%   defined in TS 38.214 Table 5.2.2.1-1, where
...
%   the PMISet. In this case, i2 values reported in PMISet are corresponds
...
%   all the precoding matrix indices. The SINR values corresponding to the
%   PMISet can be extracted by indexing using the PMISet indices.
%
...
%   SINRTable inputs respectively, in the syntax with CSI-RS object as an
%   input.
%
%   CSIRSIND are the CSI-RS indices spanning one slot. These indices are
%   1-based and are in concatenated format. It is advised to give the
...
%   (MIMO) scenario with PMI using type1 single panel codebooks.
%
%   % Example:
