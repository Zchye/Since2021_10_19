...
if isfield(simParameters, 'NumSitesPerCluster')
    cellId = simParameters.CellOfInterest;
    simParameters.NumCells = simParameters.NumSitesPerCluster;
end
...
metricsVisualizer = hNRMetricsVisualizer(simParameters, 'VisualizationFlag', visualizationFlag);

% Time step logs
if isfield(logInfo, 'TimeStepLogs') || isfield(logInfo, 'DLTimeStepLogs') || isfield(logInfo, 'ULTimeStepLogs')
    schedulingLoggerObj = hNRSchedulingLogger(simParameters, visualizationFlag, isLogReplay);
    if schedulingLoggerObj.DuplexMode % For TDD
        schedulingLoggerObj.SchedulingLog{1} = logInfo.TimeStepLogs(2:end,:); % MAC scheduling log
    else
        if visualizationFlag == 0 || visualizationFlag == 2 % Read downlink logs
            schedulingLoggerObj.SchedulingLog{1} = logInfo.DLTimeStepLogs(2:end,:); % MAC DL scheduling log
        end

        if visualizationFlag == 1 || visualizationFlag == 2 % Read uplink logs
            schedulingLoggerObj.SchedulingLog{2} = logInfo.ULTimeStepLogs(2:end,:); % MAC UL scheduling log
        end
    end
    % Add MAC metrics visualization
    addMACVisualization(metricsVisualizer, schedulingLoggerObj);
end
    ...
    for ueIdx = 1:simParameters.NumUEs
        ...
        if isfield(simParameters, 'LCHConfig')
            if isscalar(simParameters.LCHConfig.LCID)
                ...
                lchInfo(ueIdx).EntityDir = simParameters.RLCConfig.EntityDir;
            else
...
if isfield(logInfo, 'BLERLogs')
    simPhyLogger = hNRPhyLogger(simParameters, 'true');
    ...
    if isfield(logInfo, 'TimeStepLogs') || isfield(logInfo, 'DLTimeStepLogs') || isfield(logInfo, 'ULTimeStepLogs')
        if isLogReplay
            if isfield(simParameters, 'SchedulingType') && simParameters.SchedulingType
                ...
                plotPostSimRBGrids(schedulingLoggerObj, slotNum); % Plot RB-assignment grid and RB-CQI grid
            else
